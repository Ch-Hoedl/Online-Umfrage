...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (profile?.role === 'super_admin') loadPendingCount();
  }, [profile]);
  ...
  const handlePublish = async () => {
    ...
    if (Number.isNaN(expiresAtDate.getTime())) { toast.error('Ungültiges Ablaufdatum'); return; }
    ...
    if (publishMaxVotes.trim() && (!Number.isFinite(parsedMaxVotes) || (parsedMaxVotes ?? 0) < 1)) {
      toast.error('Das Stimmen-Limit muss ≥ 1 sein'); return;
    }
        ...
        <CardContent className="space-y-3">
          ...
          {isCollaborative && (
            <div className="text-xs text-green-700 bg-green-50 border border-green-100 rounded-lg px-3 py-2">
              Diese Vorlage kann von allen Benutzern gemeinsam bearbeitet werden
            </div>
          )}
          {!isCollaborative && canCopy && (
            <div className="text-xs text-purple-700 bg-purple-50 border border-purple-100 rounded-lg px-3 py-2">
              Sie können eine private Kopie erstellen und diese bearbeiten
            </div>
          )}
          <div className="flex gap-2">
            {canEdit && (
          ...
          <DialogHeader>
            ...
            <DialogDescription asChild>
              <div className="space-y-1 text-sm text-gray-600">
                ...
                <p>✅ Alle Fragen und Antwortoptionen werden kopiert</p>
                <p>✅ Die Kopie wird in "Meine Vorlagen" gespeichert</p>
                <p>✅ Sie können die Kopie frei bearbeiten</p>
              </div>
            </DialogDescription>
          </DialogHeader>
